...
router = APIRouter(prefix="/multiple-series-endpoint")

logger = get_logger("multiple-series-endpoint-logger")
...
async def predict(items: Dict[str, Dict[str, List[str]]]):
    ...
    series_one_masks = mock_up_inference(series_one_instances, radius_factor=0.1)
    series_two_masks = mock_up_inference(series_two_instances, radius_factor=0.2)
...
def generate_json_response(
    ...
    series_one_masks: np.ndarray,
    series_two_masks: np.ndarray,
    ...
    for idx, i_uid in enumerate(series_one_instances_uids):
        single_mask = {
            "points": series_one_masks[idx].tolist(),
        ...
        series_one_instances_dict[i_uid] = {"segments": segments}
